 ...
 * to inherit from this class. They must, however, implement its three instance
 * methods.
 * @property {Object} [async] - Set to specify asynchronous operations. If
 *   truthy, any `::run` call that uses this chromosome class will return a
 *   promise.
 *   @property {number} [async.fitness] - Maximum concurrency of asynchronous
 *     `#getFitness` operations. If set, `#getFitness` must return a promise.
 *     Otherwise, it must return a number as normal.
 *   @property {number} [async.crossover] - Maximum concurrency of asynchronous
 *     `#crossover` operations. If set, `#crossover` must return a promise.
 *     Otherwise, it must return a single child or array of children as normal.
 *   @property {number} [async.mutate] - Maximum concurrency of asynchronouse
 *     `#mutate' operations. If set, `#mutate` must return a promise. Otherwise,
 *     it must return a single mutant chromosome as normal.
 */
class Chromosome {
	/**
	...
	static isChromosome(obj) {
		return _.isFunction(obj.getFitness);
	}
	 ...
	 * settings.compoundCrossover option is set.
