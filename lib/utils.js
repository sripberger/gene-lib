...
/**
 * Gets an array of random chromosome indices such that some ratio of all
 * possible indices (rounded down) will always be selected.
 * @param {number} length - Length of the chromosome.
 * @param {number} [ratio=0.5] - Ratio of possible indices to select.
 * @returns {Arrray<number>} Contains a random set of integers in [0, length].
 */
exports.pickRandomIndices = function(length, ratio = 0.5) {
	return _.sampleSize(_.range(length), Math.floor(length * ratio));
};
 ...
 *   type as `left`.
 */
exports.uniformCrossover = function(left, right, ...args) {
	let indices = exports.getRandomIndices(left.length, ...args);
