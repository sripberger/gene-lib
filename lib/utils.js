...
/**
 * Gets a random chromosome index for a one-point crossover.
...
/**
 * Gets an array of chromosome indices for uniform crossover.
 * @param {number} length - Length of the chromosome.
...
/**
 * Gets an array of chromosome indices for half-uniform crossover.
 * @param {number} length - Length of the chromosome.
 * @returns {Arrray<number>} Contains a random set of indices in [0, length].
 *   Half of the possible indices rounded down will always be selected.
 */
exports.pickCrossoverIndices = function(length) {
	return _.sampleSize(_.range(length), Math.floor(length / 2));
};

/**
 * Performs a single-point crossover on two arrays or strings.
...
exports.singlePointCrossover = function(left, right) {
	let point = exports.getCrossoverPoint(left.length);
	return simpleCrossover(left, right, (i) => i >= point);
};

/**
 * Performs a two-point crossover on two arrays or strings.
 * @param {Array|String} left - Left parent.
 * @param {Array|String} right - Right parent.
 * @returns {Array<Array|String>} -  Will contain two children, of the same
 *   type as `left`.
...
exports.twoPointCrossover = function(left, right) {
	let range = exports.getCrossoverRange(left.length);
	return simpleCrossover(left, right, (i) => _.inRange(i, ...range));
};

/**
 * Performs a uniform crossover on two arrays or strings.
 * @param {Array|String} left - Left parent.
 * @param {Array|String} right - Right parent.
 * @returns {Array<Array|String>} -  Will contain two children, of the same
 *   type as `left`.
 */
exports.uniformCrossover = function(left, right) {
	let indices = exports.getCrossoverIndices(left.length);
	return simpleCrossover(left, right, (i) => _.includes(indices, i));
};

/**
 * Performs a partially-matched crossover between two parent arrays. Note that
