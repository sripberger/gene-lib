...
class Population {
	...
	static create(settings) {
		let { async } = settings;
		if (!async || !async.creation) return this.createSync(settings);
	...
	static createSync(settings) {
		...
		let individuals = _.times(
			generationSize,
			() => Individual.create(createChromosome, createArg)
		);
	...
	mutateSync() {
		...
		let mutants = this.individuals.map((i) => i.mutate(mutationRate));
