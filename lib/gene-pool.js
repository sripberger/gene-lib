...
class GenePool {
	...
	performSelections() {
		...
		if (!async || !async.selection) return this.performSelectionsSync();
	...
	performSelectionsSync() {
		...
		let crossovers = _(individuals)
			...
			.value();
		let copies = _.takeRight(individuals, this.copyCount);
		return new BreedingScheme(crossovers, copies, this.settings);
	}

	performSelectionsAsync() {
		return pasync.timesLimit(
			this.getSelectionCount(),
			this.settings.async.selection,
			() => this.selector.select()
		)
			.then((individuals) => {
				let crossovers = _(individuals)
