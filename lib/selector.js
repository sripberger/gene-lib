 ...
 *   truthy, any `::run` call that uses this selector class will return a
 *   promise.
 *   @property {number} [async.add] - Maximum concurrency of a asynchronous
 *     `#add` operations. If set, `#add` must return a promise.
 *   @property {number} [async.select] - Maximum concurrency of asynchronous
 *     `#select` operations. If set, `#select` must return a promise. Otherwise,
 *     it must return a single individual as normal.
 */
class Selector {
