...
class Individual {
	 ...
	 * @static
	 * @param {Function} chromosomeFactory - A function that should return a
	 *   chromosome object for the instance.
	 * @param {Array} [factoryArgs=[]] - Arguments for the factory function.
	 * @returns {Individual} - Created instance.
	 */
	static createSync(chromosomeFactory, factoryArgs = []) {
		return new Individual(chromosomeFactory(...factoryArgs));
	}
	 ...
	 */
	static createAsync(chromosomeFactory, factoryArgs = []) {
		return chromosomeFactory(...factoryArgs)
